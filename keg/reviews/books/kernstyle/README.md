...
K&R are **right** and (b) K&R are right.  Besides, functions are
...
Lots of people think that `typedef`s "help readability". Not so. They are
useful only for:
 ...
 (b) Clear integer types, where the abstraction **helps** avoid confusion
     ...
     `u8/u16/u32` are perfectly fine `typedef`s, although they fit into
...
fit on one or two screenfuls of text (the ISO/ANSI screen size is 80 x 24,
...
3) macros with arguments that are used as `FOO(x) = y`; will
...
only once is always a win since there is no space trade-off. While this is
...
All `EXPORT`ed functions must respect this convention, and so should all
...
going unused, mark the definition as **maybe**unused rather than wrapping it in
